...
const Register = () => {
  ...
  const onSubmit = async (formData: IRegister) => {
    ...
    if (!showVerificationInput) {
      const response = await fetch(DOMEN.user.register, {
        method: "post",
        headers: {
      ...
      const response = await fetch(DOMEN.user.verify, {
        method: "post",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  return (
    <div
      className={
        "bg-white max-w-2xl space-y-6 my-20 m-auto rounded-3xl sm:p-10"
      }
    >
      <Htag type={"h2"} className={"text-center"}>
        Регистрация
      </Htag>
      <form action="" className={"space-b-4"} onSubmit={handleSubmit(onSubmit)}>
        ...
        <Input
          ...
          {...register("confirmpassword", {
            required: { value: true, message: "Заполните confirmpassword" },
          })}
          id={"confirmpassword"}
        />
        {passwordMismatch && (
        ...
        {showVerificationInput && (
          <div className={"block"}>
            <Label htmlFor={"code"}>Код для верификации аккаунта</Label>
