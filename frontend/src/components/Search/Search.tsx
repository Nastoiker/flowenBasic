...
export const Search = ({ className, ...props }: SearchProps): JSX.Element => {
  ...
  return (
    <div className={"flex  justify-between"}>
      <div>
        <form
          className={cn(className, "w-fit ", { hidden: !isSearchVisible })}
          {...props}
          role={"search"}
        >
          <motion.div
          ...
          >
            <Input
              ...
              className={"max-w-2xl md:w-96 rounded-l-xl"}
              value={searchAlias}
              type="text"
            />
          </motion.div>
          {isFocus && founded && founded.length > 0 && (
            <div
              onFocus={() => setIsFocus(true)}
              onClick={() => setIsFocus(true)}
              className={
                " bg-white p-5 mx-auto rounded-2xl w-72 absolute z-12 my-12"
              }
            >
              {founded.map((p) => (
                <a
                  className="hover:opacity-5 block"
                  key={p.id}
                  href={"/product/phone/" + p.product[0]?.alias}
                >
                  {p.name}
                </a>
              ))}
