...
import {DeleteProductId, ICreatePhone} from "../../../interfaces/admin.interface";
...
export const DeleteModel  = (): JSX.Element => {
    ...
    const {register, control, handleSubmit, formState: {errors}, reset} = useForm<DeleteProductId>();
    const filtered = phones.filter(t => t.name.includes(search));
    return  <div className={"p-10 bg-white space-y-5 rounded-3xl text-center"}><select  className="mx-auto text-center block" {...register('id')} >
