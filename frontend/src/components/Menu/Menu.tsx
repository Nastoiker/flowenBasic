...
import {
  getSecondCategory,
  SetCurrentCategory,
} from "../../store/SecondCategory.slice";
...
export const Menu = (): JSX.Element => {
  ...
  return (
    <nav>
      {category.map((c) => {
        return (
          <div key={c.id} onClick={() => setDispatch(c.id, c.alias)}>
            {c.name}
            {c.alias === currentCategory && (
              <SecondMenu
                onShow={() => setCurrentCategory("")}
                firstCategoryId={c.id}
                firstCategoryAlias={c.alias}
              />
            )}
          </div>
        );
