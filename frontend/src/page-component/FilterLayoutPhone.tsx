...
export const FilterLayoutPhone = ({ phones, text, img }: { phones: SmartPhone[], text: string, img?: string }) => {
  ...
  const filterWithAction = () => {
   ...
   if (!phones || phones.length === 0 ) {
     return <div className={"text-center my-20 justify-self-center"}>        <Htag type={'h1'}>К сожалению ничего не найдено</Htag><Button onClick={() => redirectTo('/')}>Вернуться на главную</Button></div>;
  }
    ...
    return <div className={""}>
      ...
      <div className="flex items-center space-x-2">
        ...
        {
          (filteredPhones && filteredPhones.length > 0) ?  filteredPhones.map(p => <PhoneCard alias={p.alias} key={p.name} oldPrice={p.oldPrice} name={p.name + `\r${p.ColorAlias}`} img={ProductConvertImageNotModel(p)} price={p.price} />) :
                    <div className={"text-center my-20 justify-self-center"}>        <Htag type={'h1'}>К сожалению ничего не найдено</Htag><Button onClick={() => redirectTo('/')}>Вернуться на главную</Button></div>          
            }
        </div>
    </div>;
}