...
export const ProfileLayout = ({children}: { children: ReactNode}) => {
    ...
    return (<div className={"mx-4 my-4 mx-auto space-y-5 md:flex"}>
        <div className={"rounded-3xl block w-fit  h-fit bg-white p-2"}>
            <button className={cn("w-full   p-5 rounded-xl  hover:bg-blue-200", {
                "bg-blue-200": location.pathname ==='/Basket'
            })
            } onClick={() => redirectTo('../Basket')}>
                <Htag  className={cn("text-start", {
                    "text-white": location.pathname ==='/Basket'
                })} type={'h2'}>Корзина</Htag>
            </button>
            <button className={cn("w-full block p-5 rounded-xl  hover:bg-blue-200", {
                "bg-blue-200": location.pathname ==='/buyed'
            })
            } onClick={() => redirectTo('../basket')}>
                <Htag  className={cn("text-start", {
                    "text-white": location.pathname ==='/buyed'
                })} type={'h2'}>Купленные товары</Htag>
            </button>
            <button className={cn("w-full block p-5 rounded-xl  hover:bg-blue-200", {
                ...
                })} type={'h2'}>Комментарии</Htag>
            </button>
            <button className={cn("w-full p-5 block rounded-xl  hover:bg-blue-200", {
                "bg-blue-200": location.pathname ==='/editProfile'
            })
            } onClick={() => redirectTo('../editProfile')}>
                <Htag  className={cn("text-start", {
                    "text-white": location.pathname ==='/profile'
                })} type={'h2'}>Редактирования профиля</Htag>
            </button>
